...
int start()
{
	...
	if(AlertFlag==0){// Never Alerted
		...
		if(exADX[1]>porog1_ADX && exADX[1]<porog2_ADX && exSUB[1]>0&& exDIV[1]>0 ) {// 25<ADX<50　　かつ　ADX>遅行ADX　かつ　ADXRの傾きが正//かつ　ADXがADXRより大きい条件は抜いた
			//buy entry signal
			if(exPDI[1]>exMDI[1]){
	...
	else if(AlertFlag==1){//have some position
		//exit signal
		if(exSUB[1]<0 && exSUB[shift+1]>0 ) {// 1本前の足の終値ADXと遅行ADXの差が0未満　かつ　 2本前の足の終値ADXと遅行ADXの差が0以上　つまり、ADXが遅行ADXを下抜けした瞬間
			Alert(Symbol(),Period()," Close Signal!");
			Print("ADX-ADXs","=",exSUB[1],"-->",exSUB[shift+1]);
			AlertFlag=2;
		}
		else AlertFlag=1;
	}
